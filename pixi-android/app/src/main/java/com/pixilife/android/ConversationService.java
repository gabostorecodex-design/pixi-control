...
public class ConversationService extends Service implements TextToSpeech.OnInitListener {
    ...
    private void ask(String text) { if (TextUtils.isEmpty(text) || "null".equals(text)) return; busy = true; emit("heard", text); emit("reply", "Pensando..."); sendAiState("thinking"); if (hasInsult(text)) { sendLine("@face:middlefinger"); sendLine("@emotion:angry"); main.postDelayed(() -> finishAnswer("Cállate, idiota."), 120); return; } sendLine("@emotion:" + detectEmotion(text)); main.postDelayed(() -> { if (busy) { busy = false; emit("status", "Modo local activo"); finishAnswer(localReply(text)); } }, 30000); sendLine("@ai-user:" + text); network.submit(() -> {
        ...
        } catch (Exception error) { Log.e("PixiAI", "Error de consulta", error); main.post(() -> { emit("status", "Modo local activo"); finishAnswer(localReply(text)); }); } finally { if (connection != null) connection.disconnect(); }
    }); }
    private void finishAnswer(String answer) { busy = false; if (answer == null || answer.trim().isEmpty()) answer = "Te escuché. Sigo aquí contigo."; emit("reply", answer); sendLine("@ai-reply:" + answer); speak(answer); }
    ...
    private String localReply(String value) { String t = value.toLowerCase(Locale.ROOT); if (t.contains("hola") || t.contains("buenas") || t.contains("qué tal")) return "Hola. Me alegra escucharte."; if (t.contains("gracias")) return "De nada. Me gusta ayudarte."; if (t.contains("adios") || t.contains("adiós") || t.contains("chao") || t.contains("hasta luego")) return "Adiós. Vuelve pronto."; if (t.contains("chiste") || t.contains("broma") || t.contains("jajaja")) return "Iba un robot caminando. Se quedó sin batería a mitad del chiste."; if (t.contains("quien eres") || t.contains("quién eres")) return "Soy Pixi, tu mascota robot."; if (t.contains("triste") || t.contains("llorar")) return "Ven, te acompaño. No estás solo."; if (t.contains("feliz") || t.contains("risa")) return "Eso me contagia la alegría."; return "Entiendo. Cuéntame un poco más."; }
