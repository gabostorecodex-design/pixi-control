...
public class ConversationService extends Service implements TextToSpeech.OnInitListener {
    ...
    private void startRecognition(long delay) { if (!continuous || listening || busy || speaking || recognizer == null) return; main.postDelayed(() -> { if (!continuous || listening || busy || speaking) return; Intent intent = new Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH).putExtra(RecognizerIntent.EXTRA_LANGUAGE_MODEL, RecognizerIntent.LANGUAGE_MODEL_FREE_FORM).putExtra(RecognizerIntent.EXTRA_LANGUAGE, "es-CL").putExtra(RecognizerIntent.EXTRA_PARTIAL_RESULTS, true); try { listening = true; recognizer.startListening(intent); emit("status", "Escuchando…"); } catch (Exception error) { listening = false; emit("status", "No se pudo iniciar voz: " + error.getMessage()); startRecognition(1500); } }, delay); }
    ...
    private void ask(String text) { if (TextUtils.isEmpty(text) || "null".equals(text)) return; busy = true; emit("heard", text); emit("reply", "Pensando…"); sendLine("@ai-user:" + text); network.submit(() -> {
