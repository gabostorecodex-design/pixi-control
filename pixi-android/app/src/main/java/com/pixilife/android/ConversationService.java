...
public class ConversationService extends Service implements TextToSpeech.OnInitListener {
    ...
    private boolean continuous, listening, busy, speaking, voiceEnabled = true, wakeArmed = false;
    private final ArrayList<String[]> memory = new ArrayList<>();
    private MemoryManager memoryManager;
    private ActionExplanationManager actionLog;
    ...
    @Override public void onCreate() { super.onCreate(); memoryManager=new MemoryManager(this);actionLog=new ActionExplanationManager(this);voiceEnabled = getSharedPreferences("pixi", MODE_PRIVATE).getBoolean("voice_enabled", true); createNotificationChannel(); tts = new TextToSpeech(this, this); }

    @Override public int onStartCommand(Intent intent, int flags, int id) {
        try { ensureForeground(); } catch (SecurityException denied) { emit("status", "Concede permisos de Bluetooth, micrófono y notificaciones"); return START_NOT_STICKY; }
    ...
    private void ask(String text) { if (TextUtils.isEmpty(text) || "null".equals(text)) return; processMemoryCommand(text); busy = true; emit("heard", text); emit("reply", "Pensando..."); sendAiState("thinking"); if (hasInsult(text)) { sendLine("@face:middlefinger"); sendLine("@emotion:angry"); } else sendLine("@emotion:" + detectEmotion(text)); main.postDelayed(() -> { if (busy) { busy = false; emit("reply", "La IA tardó demasiado. Intenta nuevamente."); emit("status", "Tiempo de espera agotado"); } }, 45000); sendLine("@ai-user:" + text); network.submit(() -> {
        ...
        try {
            ...
            String payload = "{\"text\":\"" + jsonEscape(text) + "\",\"history\":" + historyJson() + ",\"memory\":\""+jsonEscape(memoryManager.context(12))+"\"}"; try (OutputStream output = connection.getOutputStream()) { output.write(payload.getBytes(StandardCharsets.UTF_8)); }
            ...
            final String result = answer.toString().trim(); Log.d("PixiAI", "Worker respuesta " + result.length() + " caracteres"); actionLog.record("voice_or_text",text,"conversation","online_answer","memoria local","OpenRouter",result.isEmpty()?"empty":"success",""); main.post(() -> finishAnswer(result));
        } catch (Exception error) { Log.e("PixiAI", "Error de consulta", error); main.post(() -> { busy=false; emit("reply", "No pude conectar con OpenRouter. Intenta nuevamente."); emit("status", "Error de OpenRouter"); }); } finally { if (connection != null) connection.disconnect(); }
    }); }
    private void finishAnswer(String answer) { busy = false; if (answer == null || answer.trim().isEmpty()) { emit("reply", "OpenRouter no devolvió texto. Intenta nuevamente."); emit("status", "Respuesta vacía"); return; } emit("reply", answer); sendLine("@ai-reply:" + answer); speak(answer); }
    ...
    private String historyJson() { StringBuilder out=new StringBuilder("["); int start=Math.max(0,memory.size()-6); for(int i=start;i<memory.size();i++){if(i>start)out.append(',');out.append("{\"role\":\"").append(memory.get(i)[0]).append("\",\"content\":\"").append(jsonEscape(memory.get(i)[1])).append("\"}");} return out.append(']').toString(); }
    private void processMemoryCommand(String value){String lower=value.toLowerCase(Locale.ROOT).trim();int p=lower.indexOf("recuerda que ");if(p>=0){String fact=value.substring(p+12).trim();if(!fact.isEmpty())memoryManager.saveMemory(lower.contains("para siempre")?MemoryManager.IMPORTANT_MEMORY:MemoryManager.LONG_TERM_MEMORY,fact,lower.contains("para siempre")||lower.contains("importante"));}if(lower.startsWith("olvida ")){org.json.JSONArray found=memoryManager.searchMemory(value.substring(7).trim());if(found.length()>0)memoryManager.deleteMemory(found.optJSONObject(0).optLong("id"));}}
    private boolean hasInsult(String value) { String t = value.toLowerCase(Locale.ROOT); String[] words = {"tonta", "tonto", "idiota", "imbecil", "estupida", "estupido", "estúpida", "estúpido", "pendeja", "pendejo", "cabrona", "cabron", "mierda", "puta", "puto", "fuck you", "fuck", "bitch", "asshole", "maldita", "maldito", "inutil", "inútil"}; for (String word : words) if (t.contains(word)) return true; return false; }
    private String detectEmotion(String value) { String t = value.toLowerCase(Locale.ROOT); if (t.contains("triste") || t.contains("llorar") || t.contains("mal")) return "sad"; if (t.contains("jaja") || t.contains("broma") || t.contains("chiste") || t.contains("feliz")) return "happy"; if (t.contains("sorpresa") || t.contains("sorprend")) return "surprised"; return "neutral"; }
    ...
    private void scanForPixi() { if (Build.VERSION.SDK_INT >= 31 && (checkSelfPermission(Manifest.permission.BLUETOOTH_SCAN) != PackageManager.PERMISSION_GRANTED || checkSelfPermission(Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED)) { emit("status", "Falta permiso de dispositivos cercanos"); return; } BluetoothManager manager = getSystemService(BluetoothManager.class); BluetoothAdapter adapter = manager == null ? null : manager.getAdapter(); if (adapter == null || !adapter.isEnabled()) { emit("status", "Activa Bluetooth"); return; } scanner = adapter.getBluetoothLeScanner(); if (scanner == null) { emit("status", "BLE no disponible"); return; } emit("status", "Buscando PIXI…"); try { scanner.startScan(scanCallback); } catch(SecurityException denied) { emit("status", "Autoriza Bluetooth cercano en Ajustes"); return; } main.postDelayed(() -> { try { if (scanner != null) scanner.stopScan(scanCallback); } catch (Exception ignored) {} }, 10000); }
