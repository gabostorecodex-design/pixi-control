...
public class ConversationService extends Service implements TextToSpeech.OnInitListener {
    ...
    private void ask(String text) { if (TextUtils.isEmpty(text) || "null".equals(text)) return; busy = true; emit("heard", text); emit("reply", "Pensando..."); sendAiState("thinking"); if (hasInsult(text)) { sendLine("@face:middlefinger"); sendLine("@emotion:angry"); } else sendLine("@emotion:" + detectEmotion(text)); main.postDelayed(() -> { if (busy) { busy = false; emit("reply", "La IA tardó demasiado. Intenta nuevamente."); emit("status", "Tiempo de espera agotado"); } }, 45000); sendLine("@ai-user:" + text); network.submit(() -> {
        ...
        try {
            ...
            String payload = "{\"text\":\"" + jsonEscape(text) + "\",\"history\":" + historyJson() + "}"; try (OutputStream output = connection.getOutputStream()) { output.write(payload.getBytes(StandardCharsets.UTF_8)); }
