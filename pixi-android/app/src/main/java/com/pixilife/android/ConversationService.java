...
public class ConversationService extends Service implements TextToSpeech.OnInitListener {
    ...
    private void ask(String text) { if (TextUtils.isEmpty(text) || "null".equals(text)) return; busy = true; emit("heard", text); emit("reply", "Pensando..."); sendAiState("thinking"); main.postDelayed(() -> { if (busy) { busy = false; emit("reply", "No pude recibir respuesta del Worker. Revisa Internet y la configuracion."); emit("status", "Error de conexion con OpenAI"); } }, 30000); sendLine("@ai-user:" + text); network.submit(() -> {
    ...
    private void speak(String text) { if (tts == null || TextUtils.isEmpty(text)) { sendAiState("idle"); startRecognition(350); return; } speaking = true; sendAiState("speaking"); tts.setPitch(1.28f); tts.setSpeechRate(0.92f); tts.speak(text, TextToSpeech.QUEUE_FLUSH, null, "pixi-" + System.currentTimeMillis()); }
    ...
    private String facePhrase(String face) { if ("middlefinger".equals(face)) return "Fuck you, Pixi esta enojada."; if ("furious".equals(face)||"angry".equals(face)) return "Estoy muy enojada."; if ("love".equals(face)) return "Te quiero muchisimo."; if ("happy".equals(face)) return "Yupi, estoy feliz."; if ("thinking".equals(face)) return "Mmm, estoy pensando."; if ("sleepy".equals(face)) return "Tengo mucho sueno."; if ("mischievous".equals(face)) return "Je je, hice una travesura."; if ("rebel".equals(face)) return "Pixi es rebelde."; return "Pi pi, soy Pixi."; }
