...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        LinearLayout voice = card(); voice.addView(label("VOZ", 13, pink)); voice.addView(label("Voz tierna de robot, equivalente al preset Tierna de la pagina: tono alto y velocidad suave.", 13, Color.LTGRAY)); Button testVoice = button("PROBAR VOZ DE PIXI", Color.rgb(31, 115, 232)); voice.addView(testVoice); testVoice.setOnClickListener(v -> startServiceAction(ConversationService.ACTION_MESSAGE, "Di: Holii, soy Pixi. Pi pi.")); root.addView(voice);
        LinearLayout profile = card(); profile.addView(label("PERFIL", 13, pink)); profile.addView(label("La personalidad y el nombre se guardan en la memoria de Pixi.", 13, Color.LTGRAY)); EditText profileName = edit("Tu nombre", false); profile.addView(profileName); Button saveProfile = button("GUARDAR PERFIL", Color.rgb(42, 155, 124)); profile.addView(saveProfile); saveProfile.setOnClickListener(v -> { startServiceAction(ConversationService.ACTION_SCAN); status.setText("Perfil listo para sincronizar con PIXI"); }); root.addView(profile);
        LinearLayout faces = card(); faces.addView(label("CARAS Y EXPRESIONES", 13, pink)); faces.addView(label("Los mismos gestos de la pagina se envian por BLE al display.", 13, Color.LTGRAY)); String[][] expressions = {{"Feliz", "happy"}, {"Cariñosa", "love"}, {"Pensando", "thinking"}, {"Molesta", "angry"}, {"Cansada", "sleepy"}, {"Robot", "robot"}, {"Traviesa", "mischievous"}, {"Fuck you", "middlefinger"}}; for (String[] item : expressions) { Button face = button(item[0], Color.rgb(30, 65, 105)); faces.addView(face); face.setOnClickListener(v -> startServiceAction(ConversationService.ACTION_FACE, item[1])); } root.addView(faces);
